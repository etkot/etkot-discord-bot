...
const SetPresence = (song) => {
    song.text_channel.client.user
        .setPresence({
            activity: { name: song.title, type: 'LISTENING' },
            status: 'online',
        })
        .catch(console.error)
}
...
const SendQueue = (channel) => {
    ...
    let totalLenght = 0
    for (let i in queue) {
        if (i < 10) str += `${Number(i) + 1}. [${queue[i].title}](${queue[i].url})\n`
    ...
    channel
        .send(new Discord.MessageEmbed().setColor('#00cc00').setTitle('Queue').setDescription(str))
        .catch(console.error)
}
...
const AddQueue = (newSong) => {
    if (playing) SendAddedToQueue(newSong)
    else SendNowPlaying(newSong)
...
const Pause = () => {
    paused = true
    if (currentDispatcher) currentDispatcher.pause()
}
const Unpause = () => {
    paused = false
    if (currentDispatcher) currentDispatcher.resume()
}
...
exports.play = {
    ...
    func: (args, msg) => {
        ...
        if (!userChannel || userChannel.guild !== msg.guild) {
            msg.channel.send("You aren't connected to a voice channel on this server").catch(console.error)
            return
        }

        if (!voiceChannel) voiceChannel = userChannel

        if (voiceChannel !== userChannel) {
            msg.channel.send('You have to be on the same voice channel').catch(console.error)
        } else {
            if (args[0]) {
                GetSong(args[0])
                    .then((song) => {
